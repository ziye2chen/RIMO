...
DEFAULT_OUTPUT_PATH = os.path.join("RIMO", "RIMO-P_solutions_qwen3_sequential.csv")
DEFAULT_MODEL = "qwen3-8b"
DEFAULT_SLEEP_SEC = 0.25


def ensure_client() -> OpenAI:
    """Construct an OpenAI-compatible client for DashScope.

    Requires environment variable DASHSCOPE_API_KEY.
    """
    if not HAS_OPENAI:
...
def read_problem_ids_from_input(input_path: str) -> List[str]:
    ...
    with open(input_path, "r", encoding="utf-8", newline="") as f:
        ...
        if "\ufeffproblem_id" in fieldnames:
            ...
            # Fallback: find a fieldname that equals problem_id after stripping BOM
...
def fix_output_problem_ids_if_needed(input_path: str, output_path: str) -> None:
    ...
    The mapping is positional: i-th output row gets i-th id from the input file.
    """
    ...
    # Rewrite output with the same columns
...
def clean_text(text: str) -> str:
    if text is None:
        return ""
    # Remove common code fences or headings and condense whitespace into single spaces
    t = text.strip()
    if t.startswith("```"):
        try:
            start = t.index("\n") + 1
            ...
            pass
    # Remove leading "Solution" style headings if present
    t = re.sub(r"^\s*#+\s*.*\n", "", t)
    t = re.sub(r"^\s*Solution[^:]*:\s*", "", t, flags=re.IGNORECASE)
    # Collapse whitespace to keep CSV tidy
...
def solve_one_step(client: OpenAI, model: str, prompt: str) -> str:
    completion = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": "You are a helpful assistant and rigorous mathematician."},
...
def process_all(
    ...
    for idx, row in enumerate(rows, 1):
        ...
        # Sequentially solve each existing sub-problem; chain the last solution forward
        ...
        for j in range(1, parts + 1):
            ...
            prompt = build_subproblem_prompt(problem_text, sub_problem, j, parts, prev_solution)
            try:
                sol = solve_one_step(client, model, prompt)
            except Exception as exc:
                sol = f"API error: {exc}"
            solutions[j - 1] = sol if sol else "N/A"
            prev_solution = sol  # carry forward as proved statement

            time.sleep(sleep_sec)

        # For non-existing parts, leave sub-problem as "None" and solution as "N/A"
...
def main(argv: List[str]) -> int:
    ...
    process_all(input_path, output_path, model, sleep_sec)
